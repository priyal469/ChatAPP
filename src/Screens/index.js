export{ default as Login} from './Login/Login';
export {default as OtpVerification} from './OtpVerification/OtpVerification';
export{default as Home} from './Home/Home';
export {default as SearchPosts} from './SearchPosts/SearchPosts';
export {default as Charts} from './Charts/Charts';
export{default as QRScanner} from './QRScanner/QRScanner';
export {default as  UserChat} from './UserChat/UserChat';
export {default as ChatInbox} from './ChatInbox/ChatInbox';